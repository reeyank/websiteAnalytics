    ...
    <div className="space-y-8">
      ...
      {showAddForm && (
        <div className="p-6 rounded-2xl bg-[var(--card)] border border-[var(--border)]">
          <h2 className="text-lg font-semibold text-[var(--foreground)] mb-4">Add New Website</h2>
          <form onSubmit={handleCreateWebsite} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl bg-[var(--background)] border border-[var(--border)] text-[var(--foreground)] placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="px-6 py-3 rounded-xl bg-[var(--background)] border border-[var(--border)] hover:border-[var(--accent)] text-[var(--foreground)] font-medium transition-colors"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="space-y-4">
        {websites.length === 0 ? (
          <div className="p-8 rounded-2xl bg-[var(--card)] border border-[var(--border)] text-center">
            <Globe className="w-12 h-12 text-[var(--muted-foreground)] mx-auto mb-4" />
            ...
            >
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-[var(--foreground)] font-semibold">{site.name}</h3>
                    <p className="text-[var(--muted-foreground)] text-sm">{site.domain}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {currentWebsite?.site_id === site.site_id && (
                    <span className="px-2 py-1 rounded-full bg-green-500/20 text-green-700 dark:text-green-400 text-xs">
                      Active
                    </span>
                  )}
              ...
              {expandedSite === site.site_id && (
                <div className="border-t border-[var(--border)] p-4 sm:p-6 space-y-6">
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl transition-colors ${
                        currentWebsite?.site_id === site.site_id
                          ...
                          : "bg-[var(--background)] border border-[var(--border)] hover:border-[var(--accent)] text-[var(--foreground)]"
                      }`}
                    >
                      {currentWebsite?.site_id === site.site_id
                        ? "Currently Active"
                  ...
                  {siteDetails[site.site_id]?.script_tag && (
                    <div>
                      ...
                      <div className="flex items-start gap-2">
                        <code className="flex-1 px-4 py-3 rounded-xl bg-[var(--background)] text-[var(--muted-foreground)] font-mono text-sm overflow-x-auto">
                          {siteDetails[site.site_id].script_tag}
                        </code>
                        <button
                          onClick={() =>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <code className="px-4 py-2 rounded-xl bg-[var(--background)] text-[var(--muted-foreground)] font-mono text-sm">
                        {site.site_id}
                      </code>
                      <button
